...
- `governance.required`: the governance cutover. With it on, an admission lands `PENDING` and stays out of `/roster` until an epoch admits it, a suspend or revoke records intent only and appends a `STATUS_INTENT` log leaf, and only a governance-signed membership epoch changes a served status. Setting it back to false is the rollback for this flag; the fail-closed policy verifier shipped with the same phase is reverted rather than flagged off (`docs/runbooks/governance-keys.md`).
