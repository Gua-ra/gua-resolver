...
2. **No endpoint ships at all until pending authority transitions have an out-of-band notification channel that is neither the account's phone number nor a session an account recovery revokes.** `DeviceNotificationService` has one implementation today and it writes a log line, which is not a channel (review finding 2). The phone is not sufficient either, because it is the channel the SIM-swap attacker holds, and sessions are not, because recovery revokes them in the same transaction that mints the factor. Every window in this record is theatre without a channel that survives both, because the owner is never told.
...
6. This record closes O9 only when its adoption path is implemented, reviewed and published under the leaf of decision 12. Until then O9 stays open and this is its proposed answer.
