...
12. **Transition flag.** `gua.resolver.governance.required`, default false. While it is false, admission and status changes take effect directly, exactly as before. When it is true, an admission lands `PENDING`, a status change records intent only, and only a governance-signed membership epoch moves an entry to ACTIVE, SUSPENDED or REVOKED.
